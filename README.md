...
- 우측 상단 **앱 설치** 버튼: Android Chrome/삼성 인터넷에서 **공식 설치 대화상자를 즉시 호출**(beforeinstallprompt → prompt()). 카카오톡 등 인앱 브라우저에서는 **'삼성 인터넷 / Chrome에서 열어 설치하기'** 큰 액션 버튼 모달(intent://, 삼성 우선 → Chrome 폴백)이 표시됩니다.
- 우측 상단 **공유** 버튼: Web Share API(`navigator.share`) 지원 환경(Android Chrome·삼성 인터넷·일반 외부 브라우저 포함)에서는 **Android 네이티브 공유 시트(카카오톡/문자/Gmail/SNS)** 를 열고, **카카오톡 등 인앱 브라우저**에서는 즉시 복사하지 않고 'Chrome/삼성 인터넷에서 열기' 안내 모달을 표시합니다. 그 외 미지원 환경에서만 기존 커스텀 공유 시트(카카오톡 / 문자 / 링크 복사) 또는 링크 복사 폴백을 사용합니다. 링크 복사 시 "링크가 복사되었습니다!" 토스트.
- 공유 대상 URL: `https://mega808-del.github.io/date/`
- PWA 매니페스트는 별도 파일 `manifest.json`을 `<head>`에서 연결합니다(Android 설치 인식에는 Data-URI 인라인 매니페스트가 동작하지 않습니다).
- **배포 시 `index.html` 과 함께 아래 파일을 같은 폴더에 업로드**하세요:
  `manifest.json` · `sw.js`(서비스 워커는 브라우저 보안 요구상 별도 파일 필수) · `icon.svg` · `icon-192.png` · `icon-512.png`
