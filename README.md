...
- 우측 상단 **앱 설치** 버튼: Android Chrome/삼성 인터넷에서 **공식 설치 대화상자를 즉시 호출**(beforeinstallprompt → prompt()). 카카오톡 등 인앱 브라우저에서는 **'Chrome/삼성 인터넷으로 열기'** 안내 모달(intent://)이 표시됩니다.
- 우측 상단 **공유** 버튼: 모바일에서 **시스템 공유(navigator.share)를 먼저 시도**하고, 미지원/차단 시 **커스텀 공유 시트(카카오톡 / 문자 / 링크 복사)** 가 열립니다. 링크 복사 시 "링크가 복사되었습니다!" 토스트.
